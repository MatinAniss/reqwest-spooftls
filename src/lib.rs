...
//! - Cookies
//!
//! The [`reqwest_spooftls::Client`][client] is asynchronous. For applications wishing
//! to only make a few HTTP requests, the [`reqwest_spooftls::blocking`](blocking) API
...
//!
//! ```rust
//! # async fn run() -> Result<(), reqwest_spooftls::Error> {
//! let body = reqwest_spooftls::get("https://www.rust-lang.org")
//!     .await?
//!     .text()
//!     .await?;
//!
...
//! type, you can use the `reqwest_spooftls::Body` constructors.
//!
//! ```rust
...
//! i.e. `let proxy = reqwest_spooftls::Proxy::http("https://secure.example")?;`
...
pub use rustls::client::Fingerprint;
pub use url::Url;
...
///
/// See also the methods on the [`reqwest_spooftls::Response`](./struct.Response.html)
/// type.
///
...
/// ```rust
/// # async fn run() -> Result<(), reqwest_spooftls::Error> {
/// let body = reqwest_spooftls::get("https://www.rust-lang.org").await?
///     .text().await?;
/// # Ok(())
/// # }
/// ```
