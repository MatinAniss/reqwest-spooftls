...
async fn test_redirect_301_and_302_and_303_changes_post_to_get() {
    ...
    for &code in &codes {
        ...
        assert_eq!(
            res.headers().get(reqwest_spooftls::header::SERVER).unwrap(),
            &"test-dst"
        );
    }
}
...
fn test_redirect_307_does_not_try_if_reader_cannot_reset() {
    ...
    for &code in &codes {
        ...
        let res = client
            .post(&url)
            .body(reqwest_spooftls::blocking::Body::new(&b"Hello"[..]))
            .send()
            .unwrap();
...
async fn test_redirect_removes_sensitive_headers() {
    ...
    reqwest_spooftls::Client::builder()
        ...
        .header(
            reqwest_spooftls::header::COOKIE,
            reqwest_spooftls::header::HeaderValue::from_static("foo=bar"),
        )
        .send()
        .await
        .unwrap();
...
async fn test_redirect_policy_can_return_errors() {
    ...
    let err = reqwest_spooftls::get(&url).await.unwrap_err();
...
async fn test_redirect_policy_can_stop_redirects_without_an_error() {
    ...
    let res = reqwest_spooftls::Client::builder()
        .redirect(reqwest_spooftls::redirect::Policy::none())
        .build()
        .unwrap()
        .get(&url)
        .send()
    ...
    assert_eq!(res.url().as_str(), url);
    assert_eq!(res.status(), reqwest_spooftls::StatusCode::FOUND);
}
...
async fn test_invalid_location_stops_redirect_gh484() {
    ...
    let res = reqwest_spooftls::get(&url).await.unwrap();
...
async fn test_redirect_302_with_set_cookies() {
    ...
    let client = reqwest_spooftls::ClientBuilder::new()
        .cookie_store(true)
        .build()
        .unwrap();
    let res = client.get(&url).send().await.unwrap();
